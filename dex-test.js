...
contract('Dex', (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([dai, bat, rep, zrx].map(
    ...
    it('should NOT deposit tokens if token does not exist', async () => {
        await expectRevert(
            await dex.deposit(web3.utils.toWei('100'), await web3.utils.fromAscii('TOKEN-DOES-NOT-EXIST'), { from: trader1 }),
    ...
    it('should withdraw tokens', async () => {
        ...
        const [balanceDex, balanceDai] = await Promise.all([
            await dex.traderBalances(trader1, DAI),
    ...
    it('should NOT withdraw tokens if token does not exist', async () => {
        await expectRevert(
            await dex.withdraw(web3.utils.toWei('100'), await web3.utils.fromAscii('TOKEN-DOES-NOT-EXIST'), { from: trader1 }),
            'this token does not exist'
        )
    })

    it('should NOT withdraw token if balance  is low', async () => {
        await dex.deposit(web3.utils.toWei('100'), DAI, { from: trader1 });
        await expectRevert(
            await dex.withdraw(web3.utils.toWei('1000'), DAI, { from: trader1 }),
    ...
    it('should NOT create limit order  if token does not exist', async () => {
        await expectRevert(
            await dex.createLimitOrder(await web3.utils.fromAscii('TOKEN-DOES-NOT-EXIST'), web3.utils.toWei('10'), 9, SIDE.BUY, { from: trader1 }),
            'this token does not exist'
        )
    })
    ...
    it('should NOT create limit order  if token is DAI', async () => {
        await expectRevert(
            await dex.createLimitOrder(DAI, web3.utils.toWei('10'), 9, SIDE.BUY, { from: trader1 }),
            'cannot trade DAI'
        )
    })
    ...
    it(' should NOT create limit order  if token balance is too low', async () => {
        ...
        await expectRevert(
            await dex.createLimitOrder(REP, web3.utils.toWei('100'), 10, SIDE.BUY, { from: trader1 }),
            'dai balance too low'
        )
    })

    it('should create market order and match against existig limit order', async () => {
        ...
        const balances = await Promise.all([
            await dex.traderBalances(trader1, DAI),
            await dex.traderBalances(trader1, REP),
            await dex.traderBalances(trader2, DAI),
            await dex.traderBalances(trader2, REP),
        ])
    ...
    it('should NOT create market  order  if token does not exist', async () => {
        await expectRevert(
            await dex.createMarketOrder(await web3.utils.fromAscii('TOKEN-DOES-NOT-EXIST'), web3.utils.toWei('1000'), SIDE.BUY, { from: trader1 }),
            'this token does not exist'
        )
    })
    ...
    it('should NOT create market  order  if token is DAI', async () => {
        await expectRevert(
            await dex.createMarketOrder(DAI, web3.utils.toWei('1000'), SIDE.BUY, { from: trader1 }),
            'cannot trade DAI'
        )
    })
    ...
    it('should NOT create market order  if token balance is too low', async () => {
        await expectRevert(
            await dex.createMarketOrder(REP, web3.utils.toWei('101'), SIDE.SELL, { from: trader2 }),
            'token balance too low'
        )
    })
    ...
    it('should NOT create market order  if dai balance is too low', async () => {
        ...
        await expectRevert(
            await dex.createMarketOrder(REP, web3.utils.toWei('101'), SIDE.BUY, { from: trader2 }),
            'dai balance too low'
        )
    })
})